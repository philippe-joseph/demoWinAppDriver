...
namespace NotepadCalculatorTest
{
    ...
    // C# Version: 7.3
    // .NET Zielversion: .NET Framework 4.8
    ...
    public class NotepadSession :IDisposable
    {
        ...
        public NotepadSession()
        {
            ...
            Assert.AreEqual("Unbenannt – Notepad", Session.Title);

            // Set implicit timeout to 1.5 seconds to make element search to retry every 500 ms for at most three times
            Session.Manage().Timeouts().ImplicitWait = TimeSpan.FromSeconds(1.5);
            NotepadMainEditBox = Session.FindElementByName("Text-Editor");
            //NotepadMainEditBox = Session.FindElementByClassName("Text-Editor");
        }
